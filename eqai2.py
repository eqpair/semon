...
REPORT_PATH   = Path("/home/ubuntu/semon/docs/data/eqai2_report.json")
...
- top_picks의 각 pick reason은 반드시 두 근거를 한 문장에 엮으세요: (1) 위 korea_impact 판단 중 어느 요인/시장 흐름에서 파생됐는지, (2) 해당 종목의 기술적 신호(스테이지2 초입, VCP 패턴, 52주 신고가 돌파, 추세·거래량 등). (예: "달러 강세 수출주 수혜 판단 + 삼성전자 스테이지2 초입·신고가 돌파")
...
<analysis>
  ...
  <strong_buy>
    <sector reason="선정근거">반도체_대형</sector>
    <sector reason="선정근거">2차전지_셀</sector>
  </strong_buy>
  <watch_sectors>
...
def _parse_xml_response(text: str, rrg_data: dict) -> dict:
    ...
    for el in root.findall("top_picks/pick"):
        ...
        if sector not in top_picks_raw:
            ...
            top_picks_raw[sector] = {"sector": sector, "reason": sr, "stocks": []}
...
def _placeholder_analysis(news_data: dict, rrg_data: dict = None) -> dict:
...
def git_push_report() -> bool:
    ...
    try:
        ...
        repo.index.add(["docs/data/eqai2_report.json"])
...
def run():
    ...
    logger.info("분석 중...")
    analysis = claude_analyze(news_data, {})
